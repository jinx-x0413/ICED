...
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FBeginActivateGizmo);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FEndActivateGizmo);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FActiveSelect);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FDeactiveSelect);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FPreviewFinished);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnGizmoPressed, FTransform, InTransform);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnGizmoReleased, FTransform, InTransform);

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnComponentActiveSelect, UPrimitiveComponent*, InComponent);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnComponentDeactiveSelect, UPrimitiveComponent*, InComponent);
...
public:
	...
	ATransformerPawn();
	virtual ~ATransformerPawn();
	virtual void GetLifetimeReplicatedProps(
		TArray<FLifetimeProperty>& OutLifetimeProps) const override;

	UPROPERTY(BlueprintAssignable)
	FBeginActivateGizmo OnBeginActivateGizmo;
	UPROPERTY(BlueprintAssignable)
	FEndActivateGizmo OnEndActivateGizmo;

	UPROPERTY(BlueprintAssignable, BlueprintCallable)
	FActiveSelect OnActiveSelect;
	UPROPERTY(BlueprintAssignable, BlueprintCallable)
	FDeactiveSelect OnDeactiveSelect;

	UPROPERTY(BlueprintAssignable)
	FPreviewFinished OnPreviewFinished;

	UPROPERTY(BlueprintCallable, BlueprintAssignable)
	FOnComponentActiveSelect OnComponentActiveSelect;
	UPROPERTY(BlueprintCallable, BlueprintAssignable)
	FOnComponentDeactiveSelect OnComponentDeactiveSelect;

	UPROPERTY(BlueprintReadWrite, EditAnywhere)
	APlayerController* mPC;

	UPROPERTY(BlueprintReadWrite, EditAnywhere)
	bool bIsActivatingGizmo;

	UPROPERTY(BlueprintReadWrite, EditAnywhere)
	bool bIsGizmoDisplayed;

	UPROPERTY(BlueprintReadWrite, EditAnywhere)
	AActor* CurrentSelectActor;

	UPROPERTY(BlueprintReadWrite, EditAnywhere)
	USkeletalMeshComponent* CurrentSelectSkmComp;
	...
	FORCEINLINE bool GetGizmoState() const;

	UFUNCTION(BlueprintCallable)
	FORCEINLINE void CallOnPreviewFinished() const;

	UFUNCTION(BlueprintCallable)
	void ForceCreateGizmo();

	UFUNCTION(BlueprintCallable)
	void CreateGizmoAtComponent(USkeletalMeshComponent* Component);

	// bIsGizmoDisplay 변수는 여기서 false로 바꿔줌
	UFUNCTION(BlueprintCallable)
	void DestroyAndClearGizmo();

	UFUNCTION(BlueprintCallable)
	void UpdateGizmoTransformations();

	UFUNCTION(BlueprintCallable)
	bool SetCurrentSelectActor(AActor* NewActor);

	UFUNCTION(BlueprintCallable)
	ETransformationType GetActiveGizmoType();
...
private:
	...
	*/
	...
	// Gizmo Command
public:
	UPROPERTY(BlueprintAssignable, EditAnywhere, Category = "Stack")
	FOnGizmoPressed OnGizmoPressed;

	UPROPERTY(BlueprintAssignable, EditAnywhere, Category = "Stack")
	FOnGizmoReleased OnGizmoReleased;

	bool bIsGizmoActivated = false;

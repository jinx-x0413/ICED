...
ATransformerPawn::ATransformerPawn()
{
	...
	bIsActivatingGizmo = false;
	bIsGizmoDisplayed = false;
}

ATransformerPawn::~ATransformerPawn()
{
	OnBeginActivateGizmo = FBeginActivateGizmo();
	OnEndActivateGizmo = FEndActivateGizmo();
	OnActiveSelect = FActiveSelect();
	OnDeactiveSelect = FDeactiveSelect();
	OnPreviewFinished = FPreviewFinished();
	OnComponentActiveSelect = FOnComponentActiveSelect();
	OnComponentDeactiveSelect = FOnComponentDeactiveSelect();
	OnGizmoPressed = FOnGizmoPressed();
	OnGizmoReleased = FOnGizmoReleased();

	mPC = nullptr;
	CurrentSelectActor = nullptr;
	CurrentSelectSkmComp = nullptr;
}
...
FORCEINLINE bool ATransformerPawn::GetGizmoState() const
{
	bool bIsGizmoShown = Gizmo.IsValid();

	return bIsGizmoShown;
}

FORCEINLINE void ATransformerPawn::CallOnPreviewFinished() const
{
	OnPreviewFinished.Broadcast();
}

void ATransformerPawn::ForceCreateGizmo()
{
	if (CurrentSelectActor && !Gizmo.IsValid() && bIsGizmoDisplayed)
	{
		if (UWorld* world = GetWorld())
		{
			...
			if (GizmoClass)
			{
				...
				Gizmo->AttachToComponent(CurrentSelectActor->GetRootComponent(), FAttachmentTransformRules::SnapToTargetIncludingScale);
			}

		}

	}
	else if (Gizmo.IsValid())
	{
		Gizmo->Destroy();
		Gizmo.Reset();
	...
	{
		// Gizmo is already exists or CurrentSelectActor is null or bool value is false

	}
}

void ATransformerPawn::CreateGizmoAtComponent(USkeletalMeshComponent* Component)
{
	...
	if (!Gizmo.IsValid())
	{
		...
		if (GizmoClass)
		{
			Gizmo = Cast<ABaseGizmo>(World->SpawnActor(GizmoClass));
			Gizmo->OnGizmoStateChange.AddDynamic(this, &ATransformerPawn::OnGizmoStateChanged);
			Gizmo->AttachToComponent(Component, FAttachmentTransformRules::SnapToTargetIncludingScale);
		}
	}
	else
	{
...
void ATransformerPawn::DestroyAndClearGizmo()
{
	if (Gizmo.IsValid())
	{
		bIsGizmoDisplayed = false;
...
void ATransformerPawn::UpdateGizmoTransformations()
{
	if (!Gizmo.IsValid())
	{
		OnEndActivateGizmo.Broadcast();
	...
	if (mPC)
	{
		...
		if (mPC->IsLocalController() && mPC->PlayerCameraManager)
		{
			if (mPC->DeprojectMousePositionToWorld(worldLocation, worldDirection))
			{
				FTransform deltaTransform = UpdateTransform(mPC->PlayerCameraManager->GetActorForwardVector()
...
bool ATransformerPawn::SetCurrentSelectActor(AActor* NewActor)
{
	bool bIsNewActor = false;

	if (CurrentSelectActor != NewActor)
	{
		if (Gizmo.IsValid())
		{
			Gizmo->Destroy();
		...
		CurrentSelectActor = NewActor;
		bIsNewActor = true;

		if (CurrentSelectActor && bIsGizmoDisplayed)
		{
			UWorld* World = GetWorld();
			if (World)
			{
				UClass* GizmoClass = GetGizmoClass(CurrentTransformation);
				Gizmo = World->SpawnActor<ABaseGizmo>(GizmoClass, FVector::ZeroVector, FRotator::ZeroRotator);
				if (Gizmo.IsValid())
				{
					Gizmo->AttachToComponent(CurrentSelectActor->GetRootComponent(), FAttachmentTransformRules::SnapToTargetIncludingScale);
				}
		...
		{
			//UE_LOG(LogTemp, Warning, TEXT("daf"));
		}
	}
...
ETransformationType ATransformerPawn::GetActiveGizmoType()
{
	ETransformationType GizmoTransform;
	if (Gizmo.IsValid())
	{
		GizmoTransform = Gizmo.Get()->GetGizmoType();

		return GizmoTransform;
	}

	return ETransformationType::TT_NoTransform;
}
...
UObject* ATransformerPawn::GetUFocusable(USceneComponent* Component) const
{
	...
	{
		// ***************************** 크래시 방지를 위해 nullptr 처리 *****************************************************
		return nullptr;
		// ***************************** 크래시 방지를 위해 nullptr 처리 *****************************************************


		AActor* ComponentOwner = Component->GetOwner();
		if (ComponentOwner && IsValid(ComponentOwner))
		{
			if (ComponentOwner->Implements<UFocusableObject>()) // error : ComponentOwner nullptr 
			{
...
void ATransformerPawn::ClearDomain()
{
	...
	// TODO : 적절한 위치로 이동?
	// AssetActor Transform Command Delegate
	if (CurrentSelectActor && bIsGizmoActivated)
	{
		OnGizmoReleased.Broadcast(CurrentSelectActor->GetTransform());
		bIsGizmoActivated = false;
	}
}
	...
	, TArray<AActor*> IgnoredActors, bool bAppendToList)
{
	...
	if (GetMouseStartEndPoints(TraceDistance, start, end))
	{
		...
		// Called when clicking on an empty space
		if (!bTraceSuccessful && !bAppendToList && !bIsModifying)
		{
			ServerDeselectAll(false);
			DestroyAndClearGizmo();
			OnDeactiveSelect.Broadcast();
		}
	...
	, const FVector& RayDirection)
{
	...
	OnBeginActivateGizmo.Broadcast();
...
{
	//UE_LOG(LogTemp, Warning, TEXT("%s "),*CurrentSelectActor->GetName());
	...
	for (auto& sc : SelectedComponents)
	{
		...
		{
			//UE_LOG(LogTemp, Warning, TEXT("Selected Component's Owner Actor: %s"), *sc->GetOwner()->GetName());
	...
	if (Gizmo.IsValid())
	{
		for (auto& hitResult : HitResults)
		{
			if (Gizmo == hitResult.GetActor()) //check if it's OUR gizmo
			{
				...
				// TODO : 책임 정의에 맞는 위치 조정
				if (CurrentSelectActor) // Delegate Callback for AssetActorTransform Command
				{
					bIsGizmoActivated = true;
					OnGizmoPressed.Broadcast(CurrentSelectActor->GetTransform());
					//GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Red, TEXT("HandleTrace Gizmo"));
				}

				UE_LOG(LogTemp, Log, TEXT("Gizmo Selected"));
				bIsActivatingGizmo = true;
				...
				if (USceneComponent* componentHit = Cast<USceneComponent>(hitResult.Component))
				{
					...
					{
						UE_LOG(LogTemp, Warning, TEXT("HandleTracedObjects : %s "), *hitResult.GetActor()->GetName());
	...
	, bool bAppendToList)
{
	if (!Actor)
	{
		// bIsModifying이 true 라면 SelectActor 호출하지 않음
		CurrentSelectActor = nullptr;
		return;
	}
	...
	{
		//CurrentSelectActor = Actor;
		UE_LOG(LogTemp, Warning, TEXT("Current Select Actor : %s"), *Actor->GetName());
		if (false == bAppendToList)
			DeselectAll();
	...
	bool bShouldSelect = ShouldSelect(Actor, Actor->GetRootComponent());
	//UE_LOG(LogTemp, Warning, TEXT("ShouldSelect for Actor %s: %s"), *Actor->GetName(), bShouldSelect ? TEXT("true") : TEXT("false"));
...
TArray<USceneComponent*> ATransformerPawn::DeselectAll(bool bDestroyDeselected)
{
	...
	{
		if (IsValid(i)) // 예외처리 추가 : 컴포넌트 유효성 체크
		{
			DeselectComponent(i);
		}
		else
		{
			UE_LOG(LogTemp, Warning, TEXT("Invalid component, skipping deselect.")); // 유효하지 않은 컴포넌트에 대한 로그 출력
		}
	}
	...
	{
		//if (i)
		//	UE_LOG(LogTemp, Warning, TEXT("DeselectComponent is %s / Owner is %s / Class is %s"), *i->GetName(), *i->GetOwner()->GetName(),*i->GetOwner()->GetClass()->GetName());
	}
	...
	, int32 Index)
{
	...
	if (OutComponentList.IsValidIndex(Index))
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("Invalid component at index %d."), Index); // 유효하지 않은 컴포넌트에 대한 로그 출력
		}
...
void ATransformerPawn::UpdateGizmoPlacement()
{
	...
	else if (SelectedComponents.Num() <= 0 && CurrentSelectActor)
	{
		//UE_LOG(LogTemp, Warning, TEXT("SelectedComponents is fewer than 0"));
		ComponentToAttachTo = CurrentSelectActor->GetRootComponent();
		if (ComponentToAttachTo)
		{
			//UE_LOG(LogTemp, Log, TEXT("Attaching gizmo to %s"), *ComponentToAttachTo->GetName());
